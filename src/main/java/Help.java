...
public class Help extends CommandBase{
        ...
        MessageEmbed[] embeds = new MessageEmbed[embedsAmount];
            ...
            for (int j = 0, k; j < 25; j++) {
                ...
                builder.addField("__" + command.CommandName() + "__",
                        command.Description() + "\n" +
                                (command.CommandAliases().length == 0 ? "" : "-# Aliases: `" + Arrays.toString(command.CommandAliases()) + "`\n") +
    ...
    public void onMessageReceived(MessageReceivedEvent event) {
        event.getChannel().sendMessageEmbeds(Arrays.asList(getEmbed())).queue();}
    public void onSlashCommandInteraction(SlashCommandInteractionEvent event) {
        event.replyEmbeds(Arrays.asList(getEmbed())).queue();}
}